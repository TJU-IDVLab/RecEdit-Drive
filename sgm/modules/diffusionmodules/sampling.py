...
class BaseDiffusionSampler:
    ...
    def prepare_sampling_loop(self, x, cond, uc=None, num_steps=None, scale_input=True):
        ...
        s_in = torch.ones(x.shape[0]).to(self.device)

        return x, s_in, sigmas, num_sigmas, cond, uc

    def denoise(self, x, x_3d, denoiser, sigma, sigma_3d, cond, cond_3d, uc, uc_3d):
        denoised, denoised_3d, e_sv3d_time2, e_x_time2 = denoiser(
        ...
        return denoised, denoised_3d, e_sv3d_time2, e_x_time2
...
class EDMSampler(SingleStepDiffusionSampler):
    ...
    def sampler_step(self, sigma, next_sigma, sigma_3d, next_sigma_3d, denoiser, x, x_3d, cond, cond_3d,
                     uc=None, uc_3d=None, gamma=0.0, z=None, z_3d=None, mask_fuse=None, copy_bg_flag=False, sv3d_model='interpolation'):
        ...
        denoised, denoised_3d, e_sv3d_time2, e_x_time2 = self.denoise(x, x_3d, denoiser, sigma_hat, sigma_hat_3d, cond, cond_3d, uc, uc_3d)
        ...
        if copy_bg_flag:
            def get_noised_input(
                sigmas_bc: torch.Tensor, noise: torch.Tensor, input: torch.Tensor
            ) -> torch.Tensor:
                noised_input = input + noise * sigmas_bc
                return noised_input
            sigmas_bc = append_dims(next_sigma, z.ndim)
            noised_input = get_noised_input(sigmas_bc, torch.randn_like(z), z)
            bg_mask = (mask_fuse.repeat(1, 4, 1, 1) + 1 ) / 2
            bg_mask = bg_mask[:z.shape[0]]
            x = noised_input * bg_mask + x * (1 - bg_mask)
        
        return x, x_3d, e_sv3d_time2, e_x_time2

    def __call__(self, denoiser, x, x_3d, cond, cond_3d, uc=None, uc_3d=None, num_steps=None, mask_fuse=None, z=None, z_3d=None, copy_bg=0.0, sv3d_model='interpolation'):
        ...
        for i in self.get_sigma_gen(num_sigmas):
            ...
            if copy_bg != 0:
                copy_bg_flag = False
                if i < 20 and mask_fuse is not None:
                    copy_bg_flag = True
            else:
                copy_bg_flag = False
             
            x, x_3d, sv3d_time2, x_time2 = self.sampler_step(
                s_in * sigmas[i],
                s_in * sigmas[i + 1],
                s_in_3d * sigmas[i],
                s_in_3d * sigmas[i + 1],
                ...
                z_3d=z_3d,
                mask_fuse=mask_fuse,
                copy_bg_flag=copy_bg_flag,
                sv3d_model=sv3d_model,
            )
            e_sv3d_time2 = e_sv3d_time2 + sv3d_time2
            e_x_time2 = e_x_time2 + x_time2
