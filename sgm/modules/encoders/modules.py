...
from ...modules.diffusionmodules.util import (extract_into_tensor,
                                              make_beta_schedule, zero_module)
...
class GeneralConditioner(nn.Module):
    OUTPUT_DIM2KEYS = {1: "ratio", 2: "vector", 3: "crossattn", 4: "concat", 5: "concat"}
    ...
    def forward(
        ...
        for embedder in self.embedders:
            ...
            emb_out = [emb_out]
            for emb in emb_out:
                if embedder.input_key == 'depth' and isinstance(emb, list):
                    out_key = 'depth'
                else:
...
class VideoPredictionEmbedderWithEncoder3D(AbstractEmbModel):
    def __init__(
        self,
        n_cond_frames: int,
        n_copies: int,
    ...
    def forward(  # same as above, but without the encoder
        self, vid: torch.Tensor
    ) -> Union[
        torch.Tensor,
        Tuple[torch.Tensor, torch.Tensor],
        ...
        # vid.shape = torch.Size([1, 3, 576, 576])
        if self.sigma_sampler is not None:
            b = vid.shape[0] // self.n_cond_frames
            sigmas = self.sigma_sampler(b).to(vid.device)
            if self.sigma_cond is not None:
        ...
        with torch.autocast("cuda", enabled=not self.disable_encoder_autocast):
            ...
            for n in range(n_rounds):   
                if self.is_ae:
                    out = self.encoder.encode(vid[n * n_samples : (n + 1) * n_samples]) # out.shape torch.Size([1, 4, 72, 72])
                else:
                    out = self.encoder(vid[n * n_samples : (n + 1) * n_samples])
                all_out.append(out)
...
class ConcatIdentity(AbstractEmbModel):
    def __init__(self):
        super().__init__()
...
class FeedForward(nn.Module):
    def __init__(self, dim, dim_out=None, mult=4., glu=False, dropout=0.0, use_residual=False):
        super().__init__()
        if use_residual:
            assert dim == dim_out
...
class DepthEmbedder(AbstractEmbModel):
    def __init__(
            ...
            output_channels: Tuple[int] = (320, 640, 1280, 1280),
            mid_channels: Tuple[int] = (32, 64, 128),
    ):
        super().__init__()
        self.blocks = nn.Sequential(
            ResBlock(input_channels, mid_channels[0], stride=1),
            ResBlock(mid_channels[0], mid_channels[1], stride=2),
            ResBlock(mid_channels[1], mid_channels[2], stride=2),
        )
        self.block1 = nn.Sequential(
            nn.Conv2d(mid_channels[2], output_channels[0], kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(output_channels[0]),
            nn.ReLU(inplace=True),
        )
        self.block2 = nn.Sequential(
            nn.Conv2d(output_channels[0], output_channels[1], kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(output_channels[1]),
            nn.ReLU(inplace=True),
        )
        self.block3 = nn.Sequential(
            nn.Conv2d(output_channels[1], output_channels[2], kernel_size=3, stride=2, padding=1),
            nn.BatchNorm2d(output_channels[2]),
            nn.ReLU(inplace=True),
        )
        self.block4 = nn.Sequential(
            nn.Conv2d(output_channels[2], output_channels[3], kernel_size=3, stride=2, padding=1),
...
class AttnEmbedder(AbstractEmbModel):
...
class AttnEmbedder3D(AbstractEmbModel):
    def __init__(self, model_config):
        super().__init__()
        self.model = None

    def forward(self, input):  # same as AttnEmbedder
        if isinstance(input, torch.Tensor):
            return self.model(input)
        else:
            output = []
